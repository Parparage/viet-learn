...
// Détermine le type d'exercice pour chaque carte selon le mode choisi
function buildExerciseTypes(words, type) {
  return words.map(() => {
...
const EXERCISE_LABELS = {
  'flashcard-vn-fr': '🃏 Flashcards VN→FR',
...
export default function SessionScreen({ words, packName, exerciseType, allWords, progress, silent, onBack, onComplete }) {
  ...
  const currentType = exerciseTypes[index]
  ...
  const onReview = () => { progress.mark(word.viet, 'review'); goTo(index + 1) }
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col max-w-lg mx-auto">
        ...
        <main className="flex-1 flex flex-col items-center justify-center px-6 gap-6">
          ...
          <h2 className="text-2xl font-bold text-gray-800 text-center">{total} cartes passées en revue</h2>
          <div className="flex gap-6 text-center">
            <div><p className="text-3xl font-bold text-green-600">{counts.known}</p><p className="text-sm text-gray-500">Sus</p></div>
            <div><p className="text-3xl font-bold text-orange-500">{counts.review}</p><p className="text-sm text-gray-500">À revoir</p></div>
            <div><p className="text-3xl font-bold text-gray-400">{total - counts.known - counts.review}</p><p className="text-sm text-gray-500">Non évalués</p></div>
          </div>
          {remaining > 0 && (
            <button onClick={() => onComplete(words.slice(SESSION_SIZE))} className="w-full py-4 bg-red-600 text-white font-bold rounded-xl active:scale-95 transition-transform">
              Continuer ({remaining} mots restants)
            </button>
          )}
          <button onClick={onBack} className="w-full py-4 bg-gray-100 text-gray-700 font-semibold rounded-xl active:scale-95 transition-transform">
            Retour à l'accueil
          </button>
        </main>
      </div>
  ...
  const isFlashcard = currentType === 'flashcard-vn-fr' || currentType === 'flashcard-fr-vn'
    ...
    <div className="min-h-screen bg-gray-50 flex flex-col max-w-lg mx-auto">
      ...
      <header className="bg-red-600 text-white px-4 py-3 flex items-center justify-between shadow-md">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold leading-tight">{packName}</h1>
            <p className="text-xs text-red-200">{EXERCISE_LABELS[exerciseType]}</p>
          </div>
        </div>
        <span className="text-sm font-bold">{index + 1} / {total}</span>
      </header>

      <div className="h-1.5 bg-red-100">
        <div className="h-full bg-amber-400 transition-all duration-300" style={{ width: `${((index + 1) / total) * 100}%` }} />
      </div>
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-3 gap-4">
          ...
          <>
            ...
            {isFlipped && (
              <div className="flex gap-3 w-full max-w-sm">
                <button onClick={onReview} className="flex-1 py-3.5 rounded-xl bg-orange-100 text-orange-700 font-semibold text-sm active:scale-95 transition-transform">🔁 À revoir</button>
                <button onClick={onKnown}  className="flex-1 py-3.5 rounded-xl bg-green-100 text-green-700 font-semibold text-sm active:scale-95 transition-transform">✓ Je sais !</button>
              </div>
            )}
            <div className="flex items-center gap-6">
              <button onClick={() => { if (index > 0) goTo(index - 1) }} disabled={index === 0} className="w-12 h-12 rounded-full bg-white shadow-md flex items-center justify-center text-gray-500 text-2xl disabled:opacity-30">‹</button>
              <span className="text-base font-bold text-gray-500 w-16 text-center">{index + 1} / {total}</span>
              <button onClick={() => goTo(index + 1)} className="w-12 h-12 rounded-full bg-white shadow-md flex items-center justify-center text-gray-500 text-2xl">›</button>
            </div>
          </>
        )}
